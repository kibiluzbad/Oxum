    ...
    public static class DictionaryExtensions
    {
        public static ExpandoObject Expando(this IDictionary<string, object> dictionary)
        {
            ...
            foreach (var item in dictionary)
            {
                ...
                {
                    expandoDic.Add(Inflector.Capitalize(item.Key), Expando((IDictionary<string, object>) item.Value));
                ...
                else if (item.Value is ICollection)
                {
                    ...
                    if (itemList.Count > 0)
                    {
                        expandoDic.Add(Inflector.Capitalize(item.Key), itemList);
                        alreadyProcessed = true;
                    }
                }

                if (!alreadyProcessed)
                    expandoDic.Add( Inflector.Capitalize(item.Key),item.Value);
            }
